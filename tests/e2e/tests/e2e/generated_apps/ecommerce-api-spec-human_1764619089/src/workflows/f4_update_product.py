...
class F4UpdateProductWorkflow:
    ...
    async def _execute_validate(self, step_num: int, context: WorkflowContext, **kwargs) -> Dict[str, Any]:
        ...
        Target: Product
        """
        self.logger.debug(f"Executing step {step_num}: Validate updated data (price > 0 if provided, stock >= 0 if provided)")
        
        # Check condition: price > 0, stock >= 0
        if not self._check_condition("price > 0, stock >= 0", context, **kwargs):
            self.logger.info(f"Skipping step {step_num}: condition not met")
            return {"skipped": True, "reason": "condition not met"}
    ...
    async def _execute_update(self, step_num: int, context: WorkflowContext, **kwargs) -> Dict[str, Any]:
        ...
        Target: Product
        """
        self.logger.debug(f"Executing step {step_num}: Update product with new data")
