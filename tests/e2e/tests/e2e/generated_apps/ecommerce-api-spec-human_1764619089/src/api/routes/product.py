...
async def returns_a_paginated_list_of_all_available_products__is_active___true_(
    db: AsyncSession=Depends(get_db)):
    """
    Returns a paginated list of all available products (is_active = true)
    """
    service = ProductService(db)
...
async def updates_name__description__price__stock_or_status_of_an_existing_product(
    id: UUID, product_data: ProductUpdate, db: AsyncSession=Depends(get_db)):
    """
...
async def delete_a_specific_product_by_id__inferred_best_practice_(id: UUID,
    db: AsyncSession=Depends(get_db)):
    """
...
async def custom_operation__f5__deactivate_product__inferred_from_flow_(id:
...
@router.patch('/')
async def patch_product(db: AsyncSession=Depends(get_db)):
    raise NotImplementedError('Endpoint patch_product needs implementation')
...
@router.put('/')
async def update_product(id: str, data: ProductUpdate, db: AsyncSession=
    Depends(get_db)):
    service = ProductService(db)
    existing = await service.get(id)
    if not existing:
