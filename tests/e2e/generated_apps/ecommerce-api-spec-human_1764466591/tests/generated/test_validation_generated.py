...
from src.models.schemas import ProductCreate, CustomerCreate, CartItemCreate, OrderItemCreate, OrderCreate, CartCreate
from src.models.entities import ProductEntity, CustomerEntity, CartItemEntity, OrderItemEntity, OrderEntity, CartEntity
...
class TestProductValidation:
    ...
    def test_product_stock_custom(self, valid_product_data):
        """Stock is deducted when order is created (checkout) for Product.stock."""
        # Custom validation test - implementation depends on specific rule
        product = ProductCreate(**valid_product_data)
        assert product.stock is not None

    def test_product_stock_custom(self, valid_product_data):
        """Stock is returned when order is cancelled for Product.stock."""
        # Custom validation test - implementation depends on specific rule
        product = ProductCreate(**valid_product_data)
        assert product.stock is not None
    ...
    def test_product_name_format_valid(self, valid_product_data):
        """name accepts length >= 1, length <= 255."""
        # Valid format should pass
        product = ProductCreate(**valid_product_data)
    ...
    def test_product_description_format_valid(self, valid_product_data):
        """description accepts length <= 2000."""
        # Valid format should pass
        product = ProductCreate(**valid_product_data)
        assert product.description is not None

    def test_product_description_format_invalid(self, valid_product_data):
        """description rejects invalid format."""
        data = valid_product_data.copy()
        data["description"] = "invalid_format_value"
        with pytest.raises(ValidationError) as exc:
            ProductCreate(**data)
...
class TestCustomerValidation:
    ...
    def test_customer_email_format_valid(self, valid_customer_data):
        """email accepts length >= 3, length <= 255."""
        # Valid format should pass
        customer = CustomerCreate(**valid_customer_data)
        assert customer.email is not None
    ...
    def test_customer_full_name_format_valid(self, valid_customer_data):
        """full_name accepts length >= 1, length <= 255."""
        # Valid format should pass
        customer = CustomerCreate(**valid_customer_data)
...
class TestCartItemValidation:
    ...
    def test_cartitem_product_id_custom(self, valid_cartitem_data):
        """Product must be active (is_active = true) to be added to cart for CartItem.product_id."""
        # Custom validation test - implementation depends on specific rule
        cartitem = CartItemCreate(**valid_cartitem_data)
        assert cartitem.product_id is not None

    def test_cartitem_quantity_stock_constraint(self, valid_cartitem_data):
        """Quantity must not exceed available product stock for CartItem.quantity."""
        # Custom validation test - implementation depends on specific rule
        cartitem = CartItemCreate(**valid_cartitem_data)
        assert cartitem.quantity is not None

    def test_cartitem_product_id_custom(self, valid_cartitem_data):
        """If product already in cart, increase quantity instead of creating duplicate for CartItem.product_id."""
        # Custom validation test - implementation depends on specific rule
        cartitem = CartItemCreate(**valid_cartitem_data)
        assert cartitem.product_id is not None

    def test_cartitem_unit_price_custom(self, valid_cartitem_data):
        """Unit price captures product price at the moment of adding to cart for CartItem.unit_price."""
        # Custom validation test - implementation depends on specific rule
        cartitem = CartItemCreate(**valid_cartitem_data)
        assert cartitem.unit_price is not None
...
class TestOrderItemValidation:
    ...
    def test_orderitem_unit_price_custom(self, valid_orderitem_data):
        """Unit price captures product price at the moment of purchase for OrderItem.unit_price."""
        # Custom validation test - implementation depends on specific rule
        orderitem = OrderItemCreate(**valid_orderitem_data)
        assert orderitem.unit_price is not None
...
class TestOrderValidation:
    ...
    def test_order_items_custom(self, valid_order_data):
        """Cart must not be empty before checkout for Order.items."""
        # Custom validation test - implementation depends on specific rule
        order = OrderCreate(**valid_order_data)
        assert order.items is not None

    def test_order_items_stock_constraint(self, valid_order_data):
        """All items must have sufficient stock before checkout for Order.items."""
        # Custom validation test - implementation depends on specific rule
        order = OrderCreate(**valid_order_data)
        assert order.items is not None

    def test_order_order_status_workflow_constraint(self, valid_order_data):
        """Can only pay order if status is PENDING_PAYMENT for Order.order_status."""
        # Custom validation test - implementation depends on specific rule
        order = OrderCreate(**valid_order_data)
        assert order.order_status is not None

    def test_order_order_status_workflow_constraint(self, valid_order_data):
        """Can only cancel order if status is PENDING_PAYMENT for Order.order_status."""
        # Custom validation test - implementation depends on specific rule
        order = OrderCreate(**valid_order_data)
        assert order.order_status is not None

    def test_order_total_amount_custom(self, valid_order_data):
        """Total amount is calculated automatically as SUM(unit_price × quantity) for all items for Order.total_amount."""
        # Custom validation test - implementation depends on specific rule
        order = OrderCreate(**valid_order_data)
        assert order.total_amount is not None
...
class TestCartValidation:
    ...
    def test_cart_customer_id_custom(self, valid_cart_data):
        """Each customer can only have one OPEN cart for Cart.customer_id."""
        # Custom validation test - implementation depends on specific rule
