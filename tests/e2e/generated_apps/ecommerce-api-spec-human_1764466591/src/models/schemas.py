...
class CustomerBase(BaseSchema):
    ...
    email: str = Field(..., min_length=3, max_length=255, pattern=
...
class CartBase(BaseSchema):
    ...
    customer_id: UUID
    status: Literal['open', 'checked_out'] = Field('OPEN', min_length=1,
        max_length=255)
...
class OrderBase(BaseSchema):
    ...
    order_status: Literal['pending_payment', 'paid', 'cancelled'] = Field(
        'PENDING_PAYMENT', min_length=1, max_length=255)
    payment_status: Literal['pending', 'simulated_ok', 'failed'] = Field(
        'PENDING', min_length=1, max_length=255)
