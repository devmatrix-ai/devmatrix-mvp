...
async def returns_all_orders_for_a_customer__optionally_filtered_by_status(
    customer_id: UUID,
    db: AsyncSession = Depends(get_db)
):
    """
    Returns all orders for a customer, optionally filtered by status
    """
    service = CustomerService(db)
...
async def retrieve_a_list_of_all_customers__inferred_best_practice_(
    db: AsyncSession = Depends(get_db)
):
    """
...
async def delete_a_specific_customer_by_id__inferred_best_practice_(
    id: UUID,
    db: AsyncSession = Depends(get_db)
):
    """
