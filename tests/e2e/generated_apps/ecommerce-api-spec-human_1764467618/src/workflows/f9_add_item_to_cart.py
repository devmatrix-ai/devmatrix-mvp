...
class F9AddItemToCartWorkflow:
    ...
    async def execute(self, context: WorkflowContext, **kwargs) -> Dict[str, Any]:
        ...
        try:
            ...
            step_result = await self._execute_update(6, context, **kwargs)
            result["steps"].append(step_result)
            
            # Step 7: Return updated cart with items
            step_result = await self._execute_validate(7, context, **kwargs)
            result["steps"].append(step_result)
    ...
    async def _execute_validate(self, step_num: int, context: WorkflowContext, **kwargs) -> Dict[str, Any]:
        ...
        Target: Product
        """
        self.logger.debug(f"Executing step {step_num}: Validate product exists and is active")
        
        # Check condition: Product exists and is_active = true
        if not self._check_condition("Product exists and is_active = true", context, **kwargs):
            self.logger.info(f"Skipping step {step_num}: condition not met")
            return {"skipped": True, "reason": "condition not met"}
    ...
    async def _execute_validate(self, step_num: int, context: WorkflowContext, **kwargs) -> Dict[str, Any]:
        ...
        Target: CartItem
        """
        self.logger.debug(f"Executing step {step_num}: Check if product already in cart")
        
        # Check condition: Check for existing cart item with same product
        if not self._check_condition("Check for existing cart item with same product", context, **kwargs):
            self.logger.info(f"Skipping step {step_num}: condition not met")
            return {"skipped": True, "reason": "condition not met"}
    ...
    async def _execute_create(self, step_num: int, context: WorkflowContext, **kwargs) -> Dict[str, Any]:
        ...
        Target: CartItem
        """
        self.logger.debug(f"Executing step {step_num}: If product in cart, increase quantity; otherwise create new cart item")
