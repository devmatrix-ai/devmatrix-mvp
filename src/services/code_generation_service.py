    ...
    async def _compose_patterns(
        ...
        **Phase 1 Refactoring**: Now accepts ApplicationIR as primary input with
        ...
        for category in composition_order:
            # Bug #206 Fix: Some categories use hardcoded generators (not patterns)
            # Always process these categories even if no patterns found
            ...
            if category not in patterns or not patterns[category]:
                if category not in hardcoded_categories:
                    ...
                    logger.info(f"🔧 Processing {category} with hardcoded generator (no patterns needed)")
            ...
            category_files = await self._compose_category_patterns(
                category,
                patterns.get(category, []),  # Bug #206: Pass empty list if no patterns
...
'''
    ...
    def _generate_readme_content_fallback(self, spec_or_ir, is_app_ir: bool) -> str:
        """Bug #194: Generate README.md when not found in PatternBank.

        Uses ApplicationIR or SpecRequirements to generate project-specific README.
        """
        # Extract project info from IR or spec
        if is_app_ir:
            app_ir = spec_or_ir
            project_name = getattr(app_ir, 'name', 'FastAPI Application')
            description = getattr(app_ir, 'description', 'Production-ready FastAPI application generated by DevMatrix.')

            # Extract entities
            entities = []
            if hasattr(app_ir, 'domain_model') and app_ir.domain_model:
                entities = [e.name for e in getattr(app_ir.domain_model, 'entities', [])]
            entity_details = "\n".join([f"- **{e}**: Full CRUD operations" for e in entities[:10]]) or "- Core business entities"

            # Extract endpoints
            endpoints = []
            if hasattr(app_ir, 'api_model') and app_ir.api_model:
                endpoints = getattr(app_ir.api_model, 'endpoints', [])
            endpoint_details = "\n".join([
                f"- `{getattr(ep, 'method', 'GET').upper()} {getattr(ep, 'path', '/')}`: {getattr(ep, 'summary', 'API endpoint')}"
                for ep in endpoints[:10]
            ]) or "- RESTful API endpoints"
        else:
            # Legacy SpecRequirements
            spec = spec_or_ir
            project_name = spec.metadata.get("project_name", "FastAPI Application") if hasattr(spec, 'metadata') else "FastAPI Application"
            description = spec.metadata.get("description", "Production-ready FastAPI application.") if hasattr(spec, 'metadata') else ""
            entity_details = "- Core business entities"
            endpoint_details = "- RESTful API endpoints"
...
'''
    ...
    async def _compose_category_patterns(
        ...
        **Phase 1 Refactoring**: Now accepts ApplicationIR as primary input with
        ...
        # Data Layer - Pydantic Models
            ...
            logger.info(f"🔧 Processing models_pydantic category")
            entities = get_entities()
            if entities:
                # Bug #200: Extract endpoint-specific schemas from IR
                # These come from endpoint.request_schema (e.g., inferred nested resource endpoints)
                endpoint_schemas = []
                endpoints = get_endpoints()
                logger.info(f"📝 Found {len(endpoints)} endpoints to check for request_schema")
                for ep in endpoints:
                    if hasattr(ep, 'request_schema') and ep.request_schema:
                        schema_dict = {
                            'name': ep.request_schema.name,
                            'fields': []
                        }
                        if hasattr(ep.request_schema, 'fields') and ep.request_schema.fields:
                            for field in ep.request_schema.fields:
                                # Bug #204 Fix: APISchemaField uses 'type' not 'data_type'
                                field_dict = {
                                    'name': getattr(field, 'name', 'unknown'),
                                    'type': getattr(field, 'type', 'string'),  # Fixed: was 'data_type'
                                    'required': getattr(field, 'required', True),
                                }
                                schema_dict['fields'].append(field_dict)
                        endpoint_schemas.append(schema_dict)
                        logger.info(f"📝 Extracted endpoint schema: {ep.request_schema.name} for {ep.method} {ep.path}")

                logger.info(f"📝 Total endpoint schemas extracted: {len(endpoint_schemas)}")
                if endpoint_schemas:
                    logger.info(f"📝 Endpoint schema names: {[s.get('name') for s in endpoint_schemas]}")
    ...
    async def _generate_route_with_llm(
        ...
        """
        # Convert CamelCase to snake_case correctly ({Entity} → {entity})
        import re
        ...
        # Bug #200 Fix: Collect endpoint-specific schema names for import
        # These come from endpoint.request_schema (derived from IR, not hardcoded)
        endpoint_schema_names = set()
        for ep in endpoints:
            if hasattr(ep, 'request_schema') and ep.request_schema and hasattr(ep.request_schema, 'name'):
                schema_name = ep.request_schema.name
                # Don't import if it's the standard Create/Update pattern (already imported)
                if not schema_name.endswith('Create') and not schema_name.endswith('Update'):
                    endpoint_schema_names.add(schema_name)
                elif schema_name not in [f'{entity.name}Create', f'{entity.name}Update',
                                         f'{entity.name}ItemCreate', f'{entity.name}ItemUpdate']:
                    endpoint_schema_names.add(schema_name)

        if endpoint_schema_names:
            item_imports += ', ' + ', '.join(sorted(endpoint_schema_names))
...
'''
        ...
        for ep in endpoints:
            ...
            # Bug #200 Fix: Use request_schema from IR if available (domain-agnostic)
            # The IR already contains the correct schema name - use it instead of hardcoding
            ir_schema_name = None
            ir_schema_snake = None
            if hasattr(ep, 'request_schema') and ep.request_schema:
                ir_schema_name = ep.request_schema.name
                # Convert CamelCase to snake_case for param name
                ir_schema_snake = ''.join(
                    f'_{c.lower()}' if c.isupper() else c
                    for c in ir_schema_name
                ).lstrip('_')
...
'''
                ...
                # Bug #202 Fix: Detect nested DELETE (e.g., /carts/{id}/items/{product_id})
                # For nested resources, we need to delete the child item, not the parent entity
                is_nested_delete = len(path_params) >= 2 and '/items/' in relative_path

                if is_nested_delete:
                    # Nested DELETE: Delete the child item
                    # Domain-agnostic: Use path params directly, derive FK from parent entity name
                    parent_id_param = path_params[0] if path_params else 'id'
                    child_id_param = path_params[-1] if len(path_params) > 1 else 'item_id'
                    child_entity_name = f'{entity.name}Item'
...
'''
                    ...
                    # Regular DELETE: Delete the entity by ID
                    id_param = path_params[0] if path_params else 'id'
...
'''
    ...
    def _generate_seed_db_script(self, spec_requirements) -> str:
        ...
        entities_list = []
        logger.info(f"🔍 [SEED_DEBUG] spec_requirements type: {type(spec_requirements).__name__}")
        logger.info(f"🔍 [SEED_DEBUG] has 'entities': {hasattr(spec_requirements, 'entities')}")
        logger.info(f"🔍 [SEED_DEBUG] has 'domain_model': {hasattr(spec_requirements, 'domain_model')}")

        if spec_requirements is not None:
            if hasattr(spec_requirements, 'entities'):
                entities_list = spec_requirements.entities
                logger.info(f"🔍 [SEED_DEBUG] Got {len(entities_list) if entities_list else 0} entities from .entities")
        ...
        # Skip entities with 2+ FK relationships (join/associative tables)
        ...
        def is_join_table(entity) -> bool:
            ...
            Example: {Child}Item has {parent}_id (many_to_one to {Parent}) and {ref}_id (many_to_one to {Ref}).

            one_to_many relationships are REVERSE references and should NOT be counted.
            Example: {Entity} has one_to_many to {Entity}Item (reverse of {Entity}Item.{entity}_id).
            """
        ...
        for idx, entity in enumerate(entities_list, start=1):
            ...
            # Bug #187: Get both UUIDs - primary for main tests, secondary for DELETE tests
            uuid_tuple = entity_uuids.get(entity_snake)
            logger.info(f"🔍 [SEED_DEBUG] Getting UUID for '{entity_snake}': {uuid_tuple}")
            if not uuid_tuple:
                raise ValueError(f"No UUID found for entity '{entity_snake}'. entity_uuids keys: {list(entity_uuids.keys())}")
            primary_uuid = uuid_tuple[0]
            delete_uuid = uuid_tuple[1]
            logger.info(f"🔍 [SEED_DEBUG] Will use primary={primary_uuid[-4:]}, delete={delete_uuid[-4:]}")

            # Get entity attributes from IR
            attributes = getattr(entity, 'attributes', []) or []

            # Bug #187: Generate seeds for BOTH primary and delete UUIDs
            for seed_idx, test_uuid in enumerate([primary_uuid, delete_uuid]):
                ...
                for attr in attributes:
                    # Domain-agnostic: Support both dict and object access for IR attributes
                    def get_attr(obj, key, default=None):
                        if isinstance(obj, dict):
                            return obj.get(key, default)
                        return getattr(obj, key, default)

                    attr_name = get_attr(attr, 'name')
                    if not attr_name:
                        continue
                    ...
                    # Get data type - handle both dict and object, and enum value extraction
                    raw_data_type = get_attr(attr, 'data_type', 'string')
                    if hasattr(raw_data_type, 'value'):
                        data_type = raw_data_type.value
                    else:
                        data_type = str(raw_data_type)
                    data_type_lower = data_type.lower()

                    # Bug #98 Fix: Check if nullable - seed all non-nullable fields
                    is_nullable = get_attr(attr, 'is_nullable', False)
                    default_value = get_attr(attr, 'default_value')
                    has_default = default_value is not None

                    # Get constraints (works for both dict and object)
                    constraints = get_attr(attr, 'constraints') or {}
                    if isinstance(constraints, dict):
                        enum_values = constraints.get('enum_values') or []
                    else:
                        enum_values = getattr(constraints, 'enum_values', None) or []
                    ...
                    if 'uuid' in data_type_lower:
                        # FK reference - try to find the referenced entity (always use primary UUID)
                        if '_id' in attr_name:
                            ref_entity = attr_name.replace('_id', '')
                            ref_uuid = get_fk_uuid(ref_entity)
                        ...
                        if 'email' in attr_name.lower():
                            # Bug #187: Use different email for delete variant
                            email_suffix = "_delete" if seed_idx == 1 else ""
                            field_assignments.append(f'{attr_name}="test{email_suffix}@example.com"')
                            ...
                            if precond_value and precond_value in (enum_values or []):
                                ...
                                field_assignments.append(f'{attr_name}="{attr_name.upper()}_INITIAL"')
                        ...
                        # Priority: 1) First enum value from IR, 2) default_value from IR
                        if enum_values and len(enum_values) > 0:
                            field_assignments.append(f'{attr_name}="{enum_values[0]}"')
                        elif has_default and isinstance(default_value, str):
                            field_assignments.append(f'{attr_name}="{default_value}"')
                        else:
                            # Last resort: use field name as value (still domain-agnostic)
                            field_assignments.append(f'{attr_name}="{attr_name.upper()}_DEFAULT"')
                ...
                if len(field_assignments) > 1:
                    ...
                    # Bug #189: Use merge() instead of add() to avoid rollback on duplicate key
                    # merge() does upsert - updates if exists, inserts if not
                    logger.info(f"🔍 [SEED_DEBUG] Adding {entity_name} seed with uuid={test_uuid[-4:]}")
                    seed_blocks.append(f'''            # Seed {entity_name} {purpose} with predictable UUID
            from src.models.entities import {entity_class}
            from uuid import UUID
            test_{entity_snake}{seed_suffix} = {entity_class}(
                {fields_str}
            )
            await session.merge(test_{entity_snake}{seed_suffix})
            logger.info("✅ Created/updated test {entity_name} with ID {test_uuid}")''')

        # Phase 1: Domain-agnostic join table seeding
        # Seed join tables AFTER parent entities, using IR attributes (not hardcoded fields)
        ...
        for entity in item_entities:
            ...
            # Get entity attributes from IR (same logic as regular entities)
            ...
            for seed_idx in range(2):
                ...
                # Build field assignments from actual IR attributes (domain-agnostic)
                field_assignments = []
                field_assignments.append(f'id=UUID("{item_uuid}")')
                needs_decimal = False

                for attr in attributes:
                    # Domain-agnostic: Support both dict and object access
                    def get_attr(obj, key, default=None):
                        if isinstance(obj, dict):
                            return obj.get(key, default)
                        return getattr(obj, key, default)
                    ...
                    if not attr_name:
                        continue
                    # Skip id (already set), created_at/updated_at (auto-generated)
                    if attr_name in ('id', 'created_at', 'updated_at'):
                        continue
                    ...
                    # Get constraints for enum values
                    constraints = get_attr(attr, 'constraints') or {}
                    if isinstance(constraints, dict):
                        enum_values = constraints.get('enum_values') or []
                    else:
                    ...
                    # Skip fields that are both nullable AND have defaults
                    if is_nullable and has_default:
                        continue
                    ...
                    if 'uuid' in data_type_lower:
                        if '_id' in attr_name:
                            ref_entity = attr_name.replace('_id', '')
                            # Use delete variant UUID for delete seeds
                            ref_uuids = entity_uuids.get(ref_entity.lower())
                            if ref_uuids:
                                ref_uuid = ref_uuids[1] if seed_idx == 1 else ref_uuids[0]
                            else:
                                ref_uuid = f"{uuid_base}1"
                            field_assignments.append(f'{attr_name}=UUID("{ref_uuid}")')
                        else:
                            field_assignments.append(f'{attr_name}=UUID("{uuid_base}9")')
                    elif 'string' in data_type_lower or 'str' in data_type_lower or 'varchar' in data_type_lower:
                        if 'email' in attr_name.lower():
                            ...
                            # Domain-agnostic status handling
                            if enum_values:
                                field_assignments.append(f'{attr_name}="{enum_values[0]}"')
                            elif has_default and isinstance(default_value, str):
                                field_assignments.append(f'{attr_name}="{default_value}"')
                ...
                if len(field_assignments) > 1:
                    fields_str = ',\n                '.join(field_assignments)
                    decimal_import = "\n            from decimal import Decimal" if needs_decimal else ""
                    seed_blocks.append(f'''            # Seed {entity_name} {purpose}
            from src.models.entities import {entity_class}
            from uuid import UUID{decimal_import}
            test_{entity_snake}{seed_suffix} = {entity_class}(
                {fields_str}
            )
            await session.merge(test_{entity_snake}{seed_suffix})
            logger.info("✅ Created/updated test {entity_name} with ID {item_uuid}")''')

        seed_code = "\n".join(seed_blocks) if seed_blocks else "            pass  # No entities to seed"
        logger.info(f"🔍 [SEED_DEBUG] Final seed_blocks count: {len(seed_blocks)}")
