...
'''
                ...
                if operation:
                    ...
                    if operation == 'add_item':
                        ...
                        body += f'''    {entity_snake} = await service.add_item({id_param}, {entity_snake}_data.model_dump() if hasattr({entity_snake}_data, 'model_dump') else {entity_snake}_data)
...
services:
  # Bug #85: DB init service runs migrations + seeds test data
  db-init:
    build:
      context: ..
    ...
    def _generate_seed_db_script(self, spec_requirements) -> str:
        ...
        for idx, entity in enumerate(entities_list, start=1):
            ...
            for attr in attributes:
                ...
                # Check if nullable - only seed required fields
                is_nullable = getattr(attr, 'nullable', True)
                has_default = getattr(attr, 'default_value', None) is not None

                # Skip nullable fields with defaults (they're optional)
