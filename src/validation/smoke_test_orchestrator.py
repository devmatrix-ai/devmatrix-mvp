...
class SmokeTestOrchestrator:
    ...
    async def run_smoke_tests(
        ...
        try:
            ...
            # UUID Unification: Prefer seed_db.py from code_generation_service
            # (uses SeedUUIDRegistry for consistent UUIDs)
            scripts_dir = app_dir / "scripts"
            scripts_dir.mkdir(exist_ok=True)
            seed_path = scripts_dir / "seed_db.py"

            if seed_path.exists():
                logger.info("━━━ Phase 2: Using existing seed_db.py ━━━")
                logger.info(f"   Found: {seed_path} (generated by code_generation_service)")
            else:
                logger.info("━━━ Phase 2: Generate Seed Data (fallback) ━━━")
